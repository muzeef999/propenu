...
const ActivePlanCard = ({ my_subscription }: ActivePlanCardProps) => {
  ...
  const renderPlanCard = (plan: Plan) => {
    ...
    const elapsedDays = now < startDate.getTime() ? 0 : (now - startDate.getTime()) / (1000 * 60 * 60 * 24);

    const planProgress = Math.min((elapsedDays / totalDays) * 100, 100);

    const remainingDays = Math.max(Math.ceil((endDate.getTime() - now) / (1000 * 60 * 60 * 24)), 0 );

    const usageProgress = plan.total > 0 ? Math.min((plan.used / plan.total) * 100, 100) : 0;

    const isPropertyPlan = plan.unit === "properties";

    return (
      <div key={plan.code} className="w-full rounded-2xl bg-[#f4fbf6] p-7 shadow-sm card">
