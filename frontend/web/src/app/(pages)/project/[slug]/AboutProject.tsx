    ...
    <section id="about-project" className="scroll-mt-20">
      <div className="container mx-auto px-1 sm:px-4 lg:px-3">
        <div className="overflow-hidden rounded-md border border-slate-200 bg-white">
          ...
          <div
            className={`grid gap-3 px-3 py-3 sm:gap-5 sm:px-5 sm:py-5 ${imageUrl && !isExpanded
            ...
            {imageUrl && (
              <div className={isExpanded ? "flex justify-center" : ""}>
                <img
                  ...
                  className={`rounded-md object-cover ${isExpanded
                      ? "h-44 w-full max-w-2xl sm:h-72 md:h-[360px]"
                      : "h-32 w-full sm:h-44 md:h-[200px]"
                    }`}
                />
              </div>
            )}

            <div className="space-y-2.5 text-xs leading-5 text-slate-500 sm:space-y-4 sm:text-base sm:leading-6">
              {descriptionText ? (
                <>
                  {richDescription ? (
                    <div
                      className={`space-y-2 sm:space-y-3 [&_a]:text-emerald-700 [&_a]:underline [&_h1]:text-base [&_h1]:font-semibold [&_h1]:leading-6 [&_h1]:text-slate-950 sm:[&_h1]:text-xl sm:[&_h1]:leading-7 [&_h2]:text-sm [&_h2]:font-semibold [&_h2]:text-slate-950 sm:[&_h2]:text-lg [&_h3]:font-semibold [&_h3]:text-slate-950 [&_mark]:rounded-sm [&_mark]:bg-yellow-100 [&_ol]:list-decimal [&_ol]:space-y-1 [&_ol]:pl-4 sm:[&_ol]:pl-5 [&_strong]:font-semibold [&_strong]:text-slate-700 [&_ul]:list-disc [&_ul]:space-y-1 [&_ul]:pl-4 sm:[&_ul]:pl-5 ${isExpanded ? "" : "line-clamp-6 sm:line-clamp-6"
                        }`}
                      dangerouslySetInnerHTML={{ __html: richDescription }}
                    />
                  ) : (
                    <p className={isExpanded ? "" : "line-clamp-6 sm:line-clamp-6"}>
                      {plainDescription}
                    </p>
                  )}
