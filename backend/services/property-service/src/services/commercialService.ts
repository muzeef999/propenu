...
async function uploadBufferToS3Local(opts: {
  ...
  const { buffer, originalname, mimetype, propertyId, folder = "commercial" } = opts;
...
export const CommercialService = {
  async create(payload: any, files?: MulterFiles) {
    ...
    if (leaseFiles.length > 0) {
      toCreate.leaseDocuments = Array.isArray(toCreate.leaseDocuments) ? toCreate.leaseDocuments.slice() : [];
      for (const f of leaseFiles) {
        ...
        toCreate.leaseDocuments.push({ title: f.originalname, url: up.url, key: up.key, filename: f.originalname, mimetype: f.mimetype });
      }
    }
    ...
    if (firstFire) {
      ...
      toCreate.fireNOCFile = { url: up.url, key: up.key, filename: firstFire.originalname, mimetype: firstFire.mimetype };
    }
    ...
    if (firstOcc) {
      ...
      toCreate.occupancyCertificateFile = { url: up.url, key: up.key, filename: firstOcc.originalname, mimetype: firstOcc.mimetype };
    }

    const created = await Commercial.create(toCreate);
  ...
  async update(id: string, payload: any, files?: MulterFiles) {
    ...
    if ((payload.slug && payload.slug !== exAny.slug) || (payload.title && payload.title !== exAny.title)) {
      const slugSource = (payload.slug && String(payload.slug).trim()) || (payload.title as string);
      const slug = slugSource.toLowerCase().trim().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
    ...
    if (galleryFiles.length > 0) {
      ...
      for (const file of remainingFiles) {
        ...
        const emptySlotIndex = (existing as any).gallery.findIndex((e: any) => !e.url);
        if (emptySlotIndex >= 0) {
          ...
          if (slot) {
            ...
            (existing as any).gallery.push({ title: file.originalname, url: up.url, key: up.key, filename: file.originalname, order: ((existing as any).gallery.length || 0) + 1 });
          }
        } else {
          (existing as any).gallery.push({ title: file.originalname, url: up.url, key: up.key, filename: file.originalname, order: ((existing as any).gallery.length || 0) + 1 });
        }
    ...
    const leaseFiles = files?.leaseDocuments ?? [];
    if (leaseFiles.length > 0) {
      (existing as any).leaseDocuments = Array.isArray(existing.leaseDocuments) ? existing.leaseDocuments : [];
      for (const f of leaseFiles) {
        const up = await uploadBufferToS3Local({
          buffer: f.buffer,
          originalname: f.originalname,
        ...
        (existing as any).leaseDocuments.push({ title: f.originalname, url: up.url, key: up.key, filename: f.originalname, mimetype: f.mimetype });
      }
    }
    ...
    if (firstFire2) {
      ...
      (existing as any).fireNOCFile = { url: up.url, key: up.key, filename: firstFire2.originalname, mimetype: firstFire2.mimetype };
    }
    ...
    if (firstOcc2) {
      ...
      if (oldKey) await deleteS3ObjectIfExists(oldKey);
      (existing as any).occupancyCertificateFile = { url: up.url, key: up.key, filename: firstOcc2.originalname, mimetype: firstOcc2.mimetype };
    }
  ...
  async delete(id: string) {
    ...
    if ((existing as any).fireNOCFile?.key) await deleteS3ObjectIfExists((existing as any).fireNOCFile.key);
    if ((existing as any).occupancyCertificateFile?.key) await deleteS3ObjectIfExists((existing as any).occupancyCertificateFile.key);
  ...
  async incrementViews(id: string) {
    if (!mongoose.Types.ObjectId.isValid(id)) return null;
    await Commercial.findByIdAndUpdate(id, { $inc: { "meta.views": 1 } }).exec();
    return null;
  },
