...
export interface AdminLeadQuery { page?: string; limit?: string; search?: string; category?: string; projectId?: string; status?: string; source?: string; state?: string; city?: string; locality?: string; from?: string; to?: string; }
...
export const getAdminLeadDashboard = async (query: AdminLeadQuery, exportAll = false) => {
  ...
  const propertyEntries = await Promise.all((Object.keys(categoryModels) as Category[]).map(async (category) => {
    ...
    const docs = await model.find({ _id: { $in: ids } }).select("title projectName buildingName propertyCode state city locality").lean();
  ...
  const counts = filtered.reduce((acc: any, row) => { acc.byCategory[row.project.category] = (acc.byCategory[row.project.category] || 0) + 1; acc.byStatus[row.status] = (acc.byStatus[row.status] || 0) + 1; acc.bySource[row.source] = (acc.bySource[row.source] || 0) + 1; return acc; }, { byCategory: {}, byStatus: {}, bySource: {} });
  ...
  return { leads: exportAll ? filtered : filtered.slice((page - 1) * limit, page * limit), pagination: { page, limit, total: filtered.length, pages: Math.ceil(filtered.length / limit) }, summary: { total: filtered.length, ...counts }, projects, facets };
};
