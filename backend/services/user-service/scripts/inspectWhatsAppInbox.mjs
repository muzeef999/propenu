...
(async () => {
  ...
  const cloudMsgs = await db.collection("whatsappmessages").countDocuments({
    wamid: { $exists: true, $nin: [null, ""], $not: { $regex: /^log:/ } },
  });
  const logMsgs = await db.collection("whatsappmessages").countDocuments({
    wamid: { $regex: /^log:/ },
  });
  console.log({ cloudMsgs, logMsgs });
